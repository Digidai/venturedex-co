#!/bin/bash
# HISTORICAL TEST FIXTURE ONLY — retired bb-browser / Comet backend.
# Never use this fixture for production. createGscFixture enables its isolated
# test copy; the checked-in fixture cannot execute browser or ledger operations.
LEGACY_GSC_TEST_FIXTURE=0
if [ "$LEGACY_GSC_TEST_FIXTURE" != 1 ]; then
  echo "Retired GSC test fixture: production uses scripts/gsc-codex.py and the Codex in-app browser." >&2
  exit 2
fi
