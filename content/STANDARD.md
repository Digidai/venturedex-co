...
**搜索方式**：先运行 `npm run curation:plan` 查看到期复审（默认展示 10 个，`--all` 查看全部；不是处理上限）。按任务范围补充近 30 天新发现；专项清理积压无需补新候选。建议覆盖三类互补来源：公司/投资机构原始公告、原创媒体、地区媒体、行业媒体或研究机构，但不把搜索次数当作单个项目的质量门槛。记录真实查询及无结果，不强制每类入选。聚合器只作发现线索，融资事实回到原始报道或公告。不要只用英文、美元金额或少数美国科技媒体构造查询。

**产出**：按 `docs/automation/curation-decisions.md` 保存不可变身份池，每批一个 `content/curation-runs/{run_id}.json`，用 `task_run_id` 关联同一任务。10–20 个只是建议批大小，无最低候选数、最高发布数或拒绝配额；有几个逐项合格就发布几个。人类要求处理全部积压时可跨批完成，不受定时任务默认范围限制。只因真实预算或排期未完成才记 `qualified_pending`，依赖门禁未过记 `publication_blocked`。中断后恢复未完阶段，不能重复发现/收录；详见 `docs/automation/throughput-and-completion.md`。
...
必须在捕获后另做一遍最终 WebP、实际卡片和详情页复核：产品已加载（loaded）、无遮挡（unobstructed）、主要文字可读（legible）、主体完整（framing）、卡片展示完整（card）、详情展示完整（detail）。优先真实独立复核；单操作者完成时用相同真实身份及 `--review-mode second-pass`，记录具体最终图片、桌面和窄布局证据，不冒充第二人。独立性不等价，但六项质量门槛相同。真实产品控件不能误判成弹窗；不能单凭尺寸、熵或工具退出码宣称视觉合格。
...
一次 Daily 运行新增多个项目时，允许在所有项目分别通过同一套事实、品牌、research、taste 和发布门禁后合并为一个内容 commit：
...
5. **不以数量替代质量。** 每个项目必须独立通过全部门禁；不设最低候选、最高发布或拒绝配额。批量提交不免除逐项审查。
...
8. **不重复消耗旧候选。** 历史拒绝有明确触发才复审；到期 overlay 或人类明确要求的复审可分批处理。每次实际尝试记录证据、结果及下一日期；不改写或截断历史，不做同日无变化重试。
