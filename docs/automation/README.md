...
The latest explicit human task instruction sets scope above scheduled defaults, without waiving safety or factual evidence. Human-directed policy repair may update the governing documents coherently; scheduled self-tuning cannot. [Throughput and completion](throughput-and-completion.md) defines finite multi-batch execution, no numeric throughput quotas, phase-specific dependencies and separate publication/indexing outcomes.
...
If two files conflict, the higher-priority file wins. Historical learning entries are evidence, not active policy; apply subsequent corrections and do not reinstate retired limits from an old run.
...
- write every run receipt/time to automation memory; append `venturedex-learning-log.md` only for a material new lesson or correction
...
Docs-only commits should be rare. Do not create a new commit/full gate/deploy merely to record the previous release's success; final release receipts belong in external automation memory/artifacts. Governance and code changes still require their full appropriate checks.
...
Versioned mirrors are `venturedex-daily-prompt.md` and `venturedex-weekly-prompt.md`. Human-directed changes update the existing saved automation through the app tool and verify the prompt readback while preserving schedule, model, project, status and notifications. A local document edit alone is not proof that the saved automation changed.
...
- Screenshot publication additionally requires the exact-hash, six-check review ledger in `content/screenshot-reviews.json`. Follow [the screenshot protocol](screenshot-quality.md): native capture, aspect-preserving import and actual final/card/detail review. Prefer a genuine independent reviewer; same-operator review must explicitly record `second-pass` with honest identity and specific observations. Import success is not approval; changing the image invalidates its review, and missing/stale review blocks validation/build. Stage the ledger with the screenshot.
