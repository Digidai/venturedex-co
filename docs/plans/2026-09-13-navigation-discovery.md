...
- A 320px pass identified cramped legacy header controls; the narrow breakpoint and menu positioning were tightened without hiding any destinations. Re-test: document width 320px, no overflow, menu fully within viewport. At 1024px all seven desktop links remain visible without horizontal overflow.
- Keyboard traversal reaches all 12 original ticker entries, with duplicates excluded. Reset the manual scroll offset on focus exit before animation resumes; preserve manual position in reduced-motion/touch mode. Final exact production receipts are required before release acceptance.
